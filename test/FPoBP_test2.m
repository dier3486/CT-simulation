...
for ishot = 0 : Nshot
    ...
    for iview = 1:Nview
        ...
        % project
        D0 = sum(interp3(image0, interpY_rep, interpX_rep, interpZ, 'linear', 0), 2).*(abs(cs_view)*h);
        D0 = reshape(D0, effNp, Nslice_ishot*2);
...
%         P2 = interp1(channelindex, Df(:, 2:2:end), t_chn2(:), 'linear', 0);
        P12 = [interp1(channelindex, Df(:, 1:2:end), t_chn1(:), 'linear', 0)  interp1(channelindex, Df(:, 2:2:end), t_chn2(:), 'linear', 0)];
