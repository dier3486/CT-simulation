...
mid_u = double(prmflow.recon.midchannel);
...
len = length(H1);
% n_l = floor((len-Npixel)/2);
% n_r = ceil((len-Npixel)/2);

d1=dataflow.rawdata(:, 30:64:end);

% % find model
% x0 = 1:Npixel;
% w1 = double(x0*d1(x0, :)./sum(d1(x0, :)));
% [~, s_mid] = min(abs(w1-mid_u));
% a0 = double(d1(x0,s_mid));
% 
% % fillup
% a1 = zeros(len, Nview);
% a1(n_l+1:len-n_r, :) = d1(x0, :);
% x_fl = [(1:n_l)-n_l  (1:n_r)+Npixel];
% s_fl = x_fl+n_l;
% for ii = 1:Nview
%     x = fzero(@(t) alignfit(a0, w1(ii), t), w1(ii)-mid_u);
%     a1(s_fl, ii) = interp1(x0, a0, x_fl-x);
% end
% a1(isnan(a1)) = 0;

[a1, n_l] = translatefillup(d1, len, mid_u);
