...
n_0 = zeros(imagesize, imagesize, 'single', 'gpuArray');
n_pi = zeros(imagesize, imagesize, 'single', 'gpuArray');
s_tmp = false(imagesize, imagesize, 'gpuArray');
gap_self = zeros(imagesize, imagesize, 'single', 'gpuArray');
gap_neib = zeros(imagesize, imagesize, 'single', 'gpuArray');
...
beta = zeros(imagesize, imagesize, Nextslice, 'single', 'gpuArray');
...
Nshift2 = reshape(repelem([Nslice_gpu/2-1 -Nslice_gpu/2], 1, Nslice_gpu/2), 1, 1, []); 
...
%         kg_pi = reshape(Zgrid,1,1,[])./t_pi + 1/2;
        ...
        kg_self_0 = reshape(Zgrid_self,1,1,[])./t_0 + 1/2;
        kg_self_pi = reshape(Zgrid_self,1,1,[])./t_pi + 1/2;
        kg_neib_0 = reshape(Zgrid_neib,1,1,[])./t_0 + 1/2;
        kg_neib_pi = reshape(Zgrid_neib,1,1,[])./t_pi + 1/2;
...
%         Nshift2 = 0;
        Tz_0 = kg_self_0.*s_self_0 + (kg_neib_pi+Nshift1).*s_neib_pi + ((kg_self_0 + Nshift2).*(t_0./gap_self) - Nshift2).*s_gap_self;
