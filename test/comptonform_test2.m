...
f_KN = finvg./dginvg;
f_R = interp1(samplekeV, f0, samplekeV2);
...
E_KN = (dsigmaKN.*S_theta.*f_KN)*samplekeV2(:).*t;
E_R = (dsigmaR.*F_theta.^2.*f_R)*samplekeV2(:).*t;
E = E_KN+E_R;

f2 = dsigmaKN.*S_theta.*f_KN + dsigmaR.*F_theta.^2.*f_R;
E0 = f0*samplekeV(:);
