...
% for ishot = 0 : Nshot
    ...
    switch ishot
        case 0
            % FP
            img_index = [ones(1, Neighb) 1:Nslice/2 ones(1, Nedge).*(Nslice/2)];
            index_fix2diff = img_index;
            imgcenter_index = (1:Nslice/2) + Neighb;
            Nimg_shot = Nslice/2 + Neighb + Nedge;
        ...
        case Nshot
            % FP
            img_index = [[ones(1, Nedge).*(-Nslice/2+1)  -Nslice/2+1:0]+Nslice*Nshot  ones(1, Neighb).*(Nshot*Nslice)];
            index_fix2diff = img_index - Nslice*(Nshot-1/2);
            imgcenter_index = (1:Nslice/2) + Nedge;
            Nimg_shot = Nslice/2 + Neighb + Nedge;
            Nslice_ishot = Nslice/2;
        ...
        otherwise
            % FP
            img_index = [ones(1,Nedge).*(-Nslice/2+1)  -Nslice/2+1:Nslice/2  ones(1,Nedge).*(Nslice/2)] + Nslice*ishot;
            index_fix2diff = img_index - Nslice*(ishot-1/2);
            imgcenter_index = (1:Nslice) + Nedge;
            Nimg_shot = Nslice + Nedge*2;
            Nslice_ishot = Nslice;
...
%     Gu = zeros(imagesize, imagesize, Nimg_shot, 'single', 'gpuArray');
    ...
    for iiter = 1 : Niter*subview
        ...
        image_diff = reshape(image0.*0, imagesize^2, Nslice_ishot);
...
%         image_diff = reshape(image_diff, imagesize, imagesize, Nimg_shot);
        ...
        image_diff = reshape(image_diff, imagesize, imagesize, Nslice_ishot);
        ...
        % err
        Rerr(iiter, imgbk_index) = gather(sqrt(sum(reshape(r, [], Nslice_ishot).^2, 1))./imagesize_GPU^2);
