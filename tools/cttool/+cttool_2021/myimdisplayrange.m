function hp = myimdisplayrange(varargin)
...
% page
pageLabelString = 'Page:';
hPageLabel = uicontrol('Parent',hPanel,...
    'Style','text',...
    'String',pageLabelString, ...
    'Tag','displayrange label',...
    'Units','pixels',...
    'Visible',visibility,...
    'BackgroundColor',backgrndColor);
...
% page label
labelExtent = get(hPageLabel,'Extent');
pagePosLabel = [posPanel(1) posPanel(2) labelExtent(3) ...
    labelExtent(4)];
set(hPageLabel,'Position',pagePosLabel);
...
hPageIndex = uicontrol('Parent',hPanel,...
    'Style','text',...
    'Units','pixels',...
    'HorizontalAlignment','left',...
    'BackgroundColor',backgrndColor,...
...
if hImagePage.Npage>1
    hlink = linkprop([hPanel hLabel hRange hPageLabel hPageIndex], 'Visible');
else
    hlink = linkprop([hPanel hLabel hRange], 'Visible');
end
...
if nargout > 0
    ...
    function createStaticDisplayRange
        ...
        % set page index
        pageIndexString = sprintf('%d',hImagePage.pageindex);
        set(hPageIndex,'String',pageIndexString);
        sethPageIndexPosition;
        
        % reset Range label Position
        posPageIndex = get(hPageIndex,'Position');
        labelExtent = get(hLabel,'Extent');
        posLabel = [posPanel(1)+pagePosLabel(3)+posPageIndex(3)+fudge posPanel(2) labelExtent(3) labelExtent(4)];
        set(hLabel,'Position',posLabel);
...
%         update page index
        scrollListener = addlistener(hImagePage, 'pageChanged', @updateVal);

        % make sure listeners are deleted when tool goes away
        setappdata(hPanel,'updateListeners',...
            [climListener cdataMappingListener scrollListener]);
        % clear unused references to listeners (do not)
%         clear climListener cdataMappingListener scrollListener;
        ...
        function updateVal(obj,eventData) %#ok<INUSD>
            ...
            setDisplayRange;
            extent = [get(hRange,'Extent'); get(hPageIndex,'Extent')];
            oldPos = [get(hRange,'Position'); get(hPageIndex,'Position')];
            if any(extent(:,3) > oldPos(:,3))
                sethRangePosition;
                sethPageIndexPosition;
                % reset Pannel Position
                posPageIndex = get(hPageIndex,'Position');
                labelExtent = get(hLabel,'Extent');
                posLabel = [posPanel(1)+pagePosLabel(3)+posPageIndex(3)+fudge posPanel(2) labelExtent(3) labelExtent(4)];
                set(hLabel,'Position',posLabel);
    ...
    %-------------------------
    function sethPageIndexPosition

        rangeExtent = get(hPageIndex,'Extent');
        posRange = [pagePosLabel(1)+pagePosLabel(3) pagePosLabel(2) rangeExtent(3) ...
            rangeExtent(4)];
        set(hPageIndex,'Position',posRange);
    ...
    function setPanelPosition
        ...
        posPageIndex = get(hPageIndex,'Position');
        panelWidth = posLabel(3) + posRange(3) + pagePosLabel(3) + posPageIndex(3) + fudge*2;
        panelHeight = max([posLabel(4) posRange(4) pagePosLabel(4) posPageIndex(4)]);
