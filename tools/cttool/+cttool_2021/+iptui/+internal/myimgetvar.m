...
%
%  IMGETVAR(...,CUSTOM) displays an import from workspace dialog and uses
%  the double scalar CUSTOM to determine whether the dialog will enforce
%  the standard set of drop-down menu choices or provide a custom
%  drop-down. A value of 0 indicates the standard drop-down, 1 indicates an
%  RGB only drop-down, 2 indicates an Intensity only mode and 3 indicates a
%  binary only drop-down.
...
%   Copyright 2004-2015 The MathWorks, Inc.
  ...
  if mode==0
      ...
      display_panels(end+1) = createImportPanel('Binary');
      display_panels(end+1) = createImportPanel('Indexed');
      display_panels(end+1) = createImportPanel('Intensity');
  end
  display_panels(end+1) = createImportPanel('RGB');  
  ...
  function hPanel = createFilterMenu
    ...
    elseif mode==1        
        image_type_str = {getString(message('images:privateUIString:imageTypeTruecolor'))};
    elseif mode==2
        image_type_str = {getString(message('images:privateUIString:imageTypeIntensity'))};
    elseif mode==3
        image_type_str = {getString(message('images:privateUIString:imageTypeBinary'))};
    end
  ...
  function hPanel = createImportPanel(im_type)
    ...
    if mode==0
        varInd = iptui.internal.myfilterWorkspaceVars(workspace_vars,im_type);
    elseif mode==1
        varInd = iptui.internal.myfilterWorkspaceVars(workspace_vars,'colorThresholderRGB');
    elseif mode==2
        varInd = iptui.internal.myfilterWorkspaceVars(workspace_vars,'imageSegmenterIntensity');
    elseif mode==3
        varInd = iptui.internal.myfilterWorkspaceVars(workspace_vars,'binary');
    end
  ...
  function status = getVars
    ...
    colormap_is_specified = ~isempty(hCMapPanelObjs) && ~isempty(get(hCMapPanelObjs(1),'String'));
  ...
  function hOut = createCMapPanel(hParent)
    ...
    iptui.internal.setChildColorToMatchParent(hCMapPanel,hParent);
    ...
    iptui.internal.setChildColorToMatchParent(hEditCMap,hCMapPanel);
    ...
    iptui.internal.setChildColorToMatchParent(hSelectCMap,hCMapPanel);
