function hfigure = myimcontrast(handle, varargin)
...
[backgroundColor, clipPanelAPI, windowClipPanelWidth] = ...
    createWindowClipPanel(hFigFlow, imgModel, rsetMode);
...
registerModularToolWithManager(hFig,imageHandle);
    ...
    function initializeContrastTool
        ...
        [badDisplayRange, filledRange] = isDisplayRangeOutsideDataRange(imageHandle,hImageAx);
%         if badDisplayRange
%             cdata = get(imageHandle, 'CData');
%             imageRange = [double(min(cdata(:))) double(max(cdata(:)))];
...
%         end
        ...
        if badDisplayRange
            set(hPanelHist.Children, 'XLim', filledRange);
        end
...
%==========================================================
function [badValue, newlim] = isDisplayRangeOutsideDataRange(him,hax)
...
newlim = [min(clim(1), histRange(1)), max(clim(2), histRange(2))];
end
