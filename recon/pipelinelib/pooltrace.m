function pipepool = pooltrace(pipepool, nodename, nextnode, operator, branchnode, branchindex)
% recode the trace of the pools
%   dataflow.pipepool = pooltrace(dataflow.pipepool, nodename, nextnode, operator);
% or,
%   dataflow.pipepool = pooltrace(dataflow.pipepool, nodename, nextnode, operator, ...
...
if ~isempty(nodename)
    for jj = 1:length(pipepool.(nodename))
        ...
        pipepool.(nodename)(jj).trace(t+1).operator = [nodename ' ' operator];
...
if ~isempty(nextnode)
    for jj = 1:length(pipepool.(nextnode))
        ...
        pipepool.(nextnode)(jj).trace(t+1).operator = [nodename ' ' operator];
...
if nargin >= 6 && ~isempty(branchnode)
    pipepool = branchpooltrace(pipepool, nodename, nextnode, operator, branchnode, branchindex);
end
