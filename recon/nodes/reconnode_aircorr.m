...
for ifocal = 1:Nfocal
    ...
    KVmA(viewindex) = KVmA(viewindex) - ...
        airKVmA(ifocal, intp_index).*(1-intp_alpha(viewindex)) - airKVmA(ifocal, intp_index+1).*intp_alpha(viewindex);
end
...
for ifocal = 1:Nfocal
    viewindex = ifocal:Nfocal:Nview;
    rawref_ifc = referenceblock(rawref(:, viewindex), referr(:, viewindex), ...
...
function ref = referenceblock(rawref, referr, referrcut, KVmA, airprm)
% reference block
...
blk1 = conv(referr(1,:)>max(referrcut(1), 1e-5), ones(1, 2*m_blk+1));
blk2 = conv(referr(2,:)>max(referrcut(2), 1e-5), ones(1, 2*m_blk+1));
