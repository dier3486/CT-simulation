function [dataflow, prmflow, status] = reconnode_hounsfieldcorr(dataflow, prmflow, status)
% recon node, Hounsfield Units correction
...
if pipeline_onoff
    ...
    dataflow.pipepool.(carrynode).data = ...
        hounsfieldcorrKernelfuntion(dataflow.pipepool.(nextnode), dataflow.pipepool.(carrynode).data);
else
    dataflow = hounsfieldcorrKernelfuntion([], dataflow);
end
...
% Done
    ...
    % Kernel funtion
    function data = hounsfieldcorrKernelfuntion(nextpool, data)
        ...
        if isfield(prmflow.corrtable, status.nodename)
            % the Hounsfield calibration is not always necessary.
