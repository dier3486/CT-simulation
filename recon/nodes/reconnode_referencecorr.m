...
Nshot = prmflow.pipe.(nodename).Nshot;
if pipeline_onoff
...
% Done
    ...
    function [data, reflast] = refcorrKernelfuntion(data, ishot, nextpool)
        ...
        if pipeline_onoff
            ...
            % set Index_corr
            IndexCorr = [max(nextpool.AvailPoint+1, nextpool.ReadPoint), nextpool.AvailPoint + plconsol.newAvail];
            Index_union = [min(plconsol.Index_in(1), IndexCorr(1)), max(plconsol.Index_in(2), IndexCorr(2))];
            % indexes to use
            index_ref = poolindex(nextpool, Index_union);
            index_out = poolindex(nextpool, plconsol.Index_out);
            Ncorr = IndexCorr(2) - IndexCorr(1) + 1;
            index_refblk = max(1, 1 + plconsol.Index_in(1) - IndexCorr(1)) : ...
                plconsol.Index_in(2) + 1 - min(plconsol.Index_in(1), IndexCorr(1));
            index_rawref = max(1, 1 + IndexCorr(1) - plconsol.Index_in(1)) : ...
                    IndexCorr(2) + 1 - min(IndexCorr(1), plconsol.Index_in(1));
            index_corr = poolindex(nextpool, IndexCorr);
            Do2i = plconsol.Do2i;
        else
            % indexes to use
            index_ref = (1:viewpershot(ishot)) + sum(viewpershot(1:ishot-1));
            switch scan
                case 'axial'
        ...
        if GPUonoff
            [referrcut] = putinGPU(referrcut);
            % I konw the mdcorr.* has been put in GPU
        end

        % ini refblock (shall not happen)
        if ~isfield(data.rawhead, 'refblock')
            data.rawhead.refblock = false(refnumber, max(index_rawref));
        end
        ...
        if Ncorr > 0
            ...
            for ifocal = 1:Nfocal
                index_rawref_ifc = index_rawref(ifocal:Nfocal:Ncorr);
                index_corr_ifc = index_corr(ifocal:Nfocal:Ncorr);
                % referencecorr
