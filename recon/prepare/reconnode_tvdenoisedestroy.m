function [dataflow, prmflow, status] = reconnode_tvdenoisedestroy(dataflow, prmflow, status)
% destroy node if backprojection
% [dataflow, prmflow, status] = reconnode_tvdenoisedestroy(dataflow, prmflow, status);
...
% cuda detroy
if libisloaded('TVdenoise')
    % CUDA Destroy
    calllib('TVdenoise', 'OuterCudaDestroy', dataflow.buffer.(nodename).CUDA.Wdenoise, ...
        dataflow.buffer.(nodename).CUDA.devPointers.Value);
end
if libisloaded('CUpipeline')
    % free the private buffers
    calllib('CUpipeline', 'cudaworkspaceFree', dataflow.buffer.(nodename).CUDA.Wimagestruct);
    calllib('CUpipeline', 'cudaworkspaceFree', dataflow.buffer.(nodename).CUDA.imagein);
    1;
