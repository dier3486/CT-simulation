function u = BregmanTV2DL(img0, mu, lambda, u0, Crange, Niter, tol)
...
function u1 = funG(f, u, b, d, mu, lambda, K2, lenxy)
[nx, ny, ~] = size(u);
...
u1 = f + d([1 1:nx-1], :, :, 1) - d(:, :, :, 1) + d(:, [1 1:ny-1], :, 2) - d(:,:,:,2);
u1 = b([1 1:nx-1], :, :, 1) + b(:, :, :, 1) - b(:, [1 1:ny-1], :, 2) + b(:,:,:,2);
u2 = ifft2(fft2(u1, lenxy(1), lenxy(2)).*K2, 'symetric');
u2 = u2./(K2
