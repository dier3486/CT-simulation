function [u, b] = BregmanTV3D_axialiter(u, mu, Cl, b0, Crange, Niter, tol, Zbalance)
% Split Bregman method for 3D TV, tmp version for axial iteration recon
% [u, b] = BregmanTV3D_axialiter(u, mu, Cl, b0, Crange, Niter, tol, Zbalance);
...
for ii = 1:Niter
    u1 = funG(real(u), imag(u), b, d, Cl, Zbalance);
    delta(ii) = sqrt(sum((imag(u(:))-u1(:)).^2)./N1);
    u = real(u) + 1.0i.*u1;
    if delta(ii)<tol || ii == Niter
        break;
    end
...
u = (u([2:nx nx-1], :, :) + u([2 1:nx-1], :, :) + u(:, [2:ny ny-1], :) + u(:, [2 1:ny-1], :)).*balance_xy ...
