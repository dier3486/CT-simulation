function u = BregmanTV2D_tanh(img0, mu, lambda, u0, Crange, Niter, tol)
% Split Bregman method (not) for TV
...
f1 = hardentanh(f-u, 1e-3, 0.05) + u;
% f1 = f;

u1 = u1.*(lambda./(mu+4*lambda)) + f1.*(mu./(mu+4*lambda));
