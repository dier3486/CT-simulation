function [u, delta] = BregmanTV2D_oneway(img0, mu, Cl, u0, Crange, Niter, tol)
% Split Bregman method for TV 2D, oneway bondary condition
% u = BregmanTV2D_oneway(img0, mu, Cl, u0, Crange, Niter, tol)
% or u = BregmanTV2D_oneway(f0, mu, Cl);
% the Cl is lambda./mu
...
u(:, 2:end, :) = u([2:nx nx-1], 2:end, :) + u([2 1:nx-1], 2:end, :) + u(:, [3:ny ny-1], :) + u(:, 1:ny-1, :);

% u(:, 2:end, :) = u(:, 2:end, :) + d([1 1:nx-1], 2:end, :, 1) - d(:, 2:end, :, 1) + d(:, 1:ny-1, :, 2) - d(:,2:end,:,2);
...
% u(:, 2:end, :) = u(:, 2:end, :) - b([1 1:nx-1], 2:end, :, 1) + b(:, 2:end, :, 1) - b(:, 1:ny-1, :, 2) + b(:,2:end,:,2);
...
u(:, 2:end, :) = u(:, 2:end, :).*(Cl/(1+Cl*4)) + f(:, 2:end, :).*(1/(1+Cl*4));
